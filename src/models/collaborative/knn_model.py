...
class EnhancedKNNCollaborativeFilter:
    ...
    def __init__(
        ...
        Args:
            ...
            use_review_quality: Whether to weight by review helpfulness
            use_temporal_decay: Whether to apply temporal decay to older reviews
            use_user_authority: Whether to weight by user's review count
            temporal_decay_days: Days for temporal decay to reduce weight by half
        """
        self.k = k
        # Use Pearson correlation for sparse data (more robust than cosine)
        self.sim_options = sim_options or {
            'name': 'pearson',  # Changed from cosine to pearson for sparse data
            'user_based': True,
            'min_support': 2  # Minimum number of common items for similarity calculation
        }
    ...
    def _calculate_review_quality_weight(self, helpful: int, funny: int) -> float:
        """
        Calculate weight based on review quality metrics.
        ...
        Returns:
            Quality weight multiplier (0.5 to 2.0)
        """
        if not self.use_review_quality:
            return 1.0
        
        # Combine helpful and funny votes (helpful weighted more)
        quality_score = helpful * 2 + funny
        
        # Apply logarithmic scaling to prevent extreme weights
        if quality_score <= 0:
            weight = 0.8  # Slightly lower for reviews with no positive feedback
        elif quality_score <= 2:
            weight = 1.0  # Normal weight
        elif quality_score <= 10:
            weight = 1.2  # Slight boost for moderately helpful reviews
        elif quality_score <= 50:
            weight = 1.5  # Good boost for helpful reviews
        else:
            weight = 2.0  # Maximum boost for very helpful reviews
    ...
    def _calculate_temporal_weight(self, review_date: pd.Timestamp) -> float:
        """
        Calculate weight based on review recency.
        
        Args:
            review_date: Date of the review
            
        Returns:
            Temporal weight multiplier (0.1 to 1.0)
        """
        ...
        # Apply exponential decay: weight = exp(-days / decay_period)
    ...
    def _calculate_user_authority_weight(self, user_id: int) -> float:
        """
        Calculate weight based on user's review authority.
        
        Args:
            user_id: User identifier
            
        Returns:
            Authority weight multiplier (0.5 to 1.5)
        """
        ...
        if review_count <= 1:
            weight = 0.7  # Low authority for users with very few reviews
        elif review_count <= 5:
            weight = 0.9  # Slightly lower for new reviewers
        elif review_count <= 20:
            weight = 1.0  # Normal weight
        elif review_count <= 100:
            weight = 1.2  # Boost for experienced reviewers
        else:
            weight = 1.5  # Maximum boost for power reviewers
    ...
    def prepare_enhanced_data(
        ...
        """
        Prepare enhanced data with Steam-specific features.
        
        Args:
            recommendations_df: User-item interaction data with Steam features
            games_df: Game metadata
            users_df: User profile data (optional)
            
        Returns:
            Surprise Dataset object with enhanced ratings
        """
        ...
        # Apply weights to create enhanced ratings
        ...
        # Normalize enhanced ratings to [0, 1] range
        ...
        # Store enhanced ratings for analysis
        ...
        # Create reader with enhanced rating scale
    ...
    def train(self, data: Dataset) -> None:
        """
        Train the enhanced KNN collaborative filtering model.
        
        Args:
            data: Surprise Dataset object with enhanced ratings
        """
        ...
        # Adjust k based on data size to prevent issues
        max_k = min(self.k, self.trainset.n_users - 1, 50)  # Limit k for sparse data
        if max_k < self.k:
            logger.warning(f"   ⚠️  Reducing k from {self.k} to {max_k} due to data constraints")
            self.k = max_k
        
        # Choose appropriate similarity measure based on sparsity
        if sparsity > 0.99:  # Very sparse data
            logger.info("   🔧 Using MSD similarity for very sparse data")
            self.sim_options['name'] = 'msd'  # Mean Squared Difference - more robust for sparse data
        elif sparsity > 0.95:  # Sparse data
            logger.info("   🔧 Using Pearson correlation for sparse data")
            self.sim_options['name'] = 'pearson'
        
        # Initialize KNN model with robust settings
        self.model = KNNBasic(
            k=self.k,
        ...
        try:
            ...
            # Fallback to simplest similarity measure
    ...
    def predict_rating(self, user_id: int, item_id: int) -> float:
        """
        Predict enhanced rating for a user-item pair.
    ...
    def get_user_recommendations(
        ...
        """
        Get top-N enhanced recommendations for a user.
...
def train_enhanced_knn_model(
    ...
    """
    Convenience function to train an enhanced KNN collaborative filtering model.
    ...
    Args:
        ...
        use_review_quality: Whether to use review quality weighting
        use_temporal_decay: Whether to use temporal decay
        use_user_authority: Whether to use user authority weighting
        
    Returns:
        Trained EnhancedKNNCollaborativeFilter model
    """
...
# Keep the original class for backward compatibility, but mark as deprecated
class KNNCollaborativeFilter(EnhancedKNNCollaborativeFilter):
    """
    Backward compatibility wrapper for the enhanced KNN model.
    """
    ...
    def __init__(self, k: int = 40, sim_options: Optional[Dict] = None):
        ...
        super().__init__(
            ...
            use_review_quality=False,  # Disable enhancements for backward compatibility
