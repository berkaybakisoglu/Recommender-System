"""
Simple Hybrid Recommendation System without scikit-surprise dependency.
"""
...
from src.models.collaborative.simple_knn import SimpleKNNCollaborativeFilter
...
class SimpleHybridRecommender:
    """
    Simple hybrid recommendation system combining collaborative and content-based filtering.
    """
    ...
    def __init__(
        ...
        """
        Initialize simple hybrid recommendation system.
    ...
    def train(
        ...
        """
        logger.info("Training simple hybrid recommendation system...")
        ...
        self.cf_model = SimpleKNNCollaborativeFilter(**cf_params)
        self.cf_model.train(recommendations_df, games_df)
        ...
        logger.info("Simple hybrid recommendation system training completed")
...
def train_simple_hybrid_model(
    games_df: pd.DataFrame,
    recommendations_df: pd.DataFrame,
    games_metadata: Dict,
    cf_weight: float = 0.6,
    cb_weight: float = 0.4
) -> SimpleHybridRecommender:
    """
    Convenience function to train a simple hybrid recommendation model.
    ...
    Returns:
        Trained SimpleHybridRecommender model
    """
    model = SimpleHybridRecommender(cf_weight=cf_weight, cb_weight=cb_weight)
...
if __name__ == "__main__":
    ...
    # Train simple hybrid model
    hybrid_model = train_simple_hybrid_model(
        games_df, recommendations_df, games_metadata,
        cf_weight=0.6, cb_weight=0.4
    )
    
    # Get model info
    print("Simple Hybrid Model Info:", hybrid_model.get_model_info())
    
    # Get recommendations for user 1
    recommendations = hybrid_model.get_user_recommendations(user_id=1, n_recommendations=3)
    print(f"Simple hybrid recommendations for User 1:")
